...
class LoadAgent {
    ...
    static class LoadTransformer implements ClassFileTransformer {
        ...
        public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classBytes) {
            /* Don't transform net/adoptopenjdk/loadTest/LoadTest */
            // TODO needed?
            if (className.contains("net/adoptopenjdk/loadTest/LoadTest")) {
                return null;
            }
            
            // TODO do logging for this
        ...
        /* TODO if I change this all to use asm I can do it for all versions
                ...
                methodModel -> methodModel.code()
                                        .map(codeModel ->
                                                codeModel.elementStream()
                                                        .anyMatch(LoadTransformer::isInvocationOfSystemExit))
                                        .orElse(false);

            CodeTransform rewriteSystemExit =
                (codeBuilder, codeElement) -> {
                    if (isInvocationOfSystemExit(codeElement)) {
                        var myexception = ClassDesc.of("net.adoptopenjdk.loadTest.BlockedExitException");
                        codeBuilder.new_(myexception)                    
                                .dup()
                                .ldc("System.exit not allowed")
                                .invokespecial(myexception,
                                    "<init>",
